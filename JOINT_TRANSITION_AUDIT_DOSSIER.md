...
### 5.1 Setup — Live Verified

```
Command: pip install -e ".[dev]"
Result:  Successfully installed flask-3.1.3 gunicorn-26.0.0 mypy-2.1.0 ruff-0.15.15
         werkzeug-3.1.8 blinker-1.9.0 itsdangerous-2.2.0 + blockage-analyzer-1.0.0
Platform: win32, Python 3.13.13
```

### 5.2 Live Execution Evidence — Verified 2026-06-02

All three validations run directly via Python pipeline (no HTTP server required for evidence):

**Valid payload output (live run):**
```json
{
  ...
  "impact_score": 10,
  "severity": "HIGH",
  "timestamp": "2026-06-02T04:06:34Z"
}
```

**Malformed payload rejection (missing trace_id — live run):**
```
status:  FAIL
output:  {"status": "FAIL", "reason": "INVALID_INPUT_CONTRACT", "trace_id": ""}
stderr:  insightflow | {'type': 'FAILURE', 'trace_id': '', 'reason': 'INVALID_INPUT_CONTRACT'}
```
...
**Result:** 123 passed in 0.72s — verified live on 2026-06-02, Python 3.13.13, win32
...
### 6.2 Coverage — Live Verified

```
Name                                Stmts   Miss  Cover   Missing
-----------------------------------------------------------------
analyzer\__init__.py                    2      0   100%
analyzer\action_generator.py            8      0   100%
analyzer\analyze_blockage.py           43      0   100%
analyzer\blocked_task_detector.py       4      0   100%
analyzer\bottleneck_detector.py         8      0   100%
analyzer\output_structurer.py          17      0   100%
analyzer\root_cause_tracer.py          36      0   100%
tantra\__init__.py                      2      0   100%
tantra\bucket.py                       29      0   100%
tantra\core.py                          6      0   100%
tantra\insightflow.py                  26      0   100%
tantra\pipeline.py                     24      0   100%
tantra\rajya.py                         9      0   100%
tantra\sarathi.py                       7      0   100%
-----------------------------------------------------------------
TOTAL                                 221      0   100%
...
### 6.3 Lint and Type Check — Live Verified
...
## Section 11 — Runtime Evidence Appendix (Live Verified 2026-06-02)

All evidence captured from live execution on win32, Python 3.13.13.

**Environment setup:**
```
pip install -e ".[dev]"
Successfully installed flask-3.1.3 gunicorn-26.0.0 mypy-2.1.0 ruff-0.15.15
                     werkzeug-3.1.8 blinker-1.9.0 itsdangerous-2.2.0
                     blockage-analyzer-1.0.0
```
...
```
Input:   {"execution_id": "exec-bad"}  [missing trace_id]
Status:  FAIL
Output:  {"status": "FAIL", "reason": "INVALID_INPUT_CONTRACT", "trace_id": ""}
InsightFlow: {"type": "FAILURE", "trace_id": "", "reason": "INVALID_INPUT_CONTRACT"}
```
